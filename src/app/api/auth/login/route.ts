...
export async function POST(req: Request) {
  ...
  try {
    ...
    res.cookies.set(SESSION_COOKIE, token, {
      ...
      // على Vercel (HTTPS) نجعل الكوكي مؤمّنًا فقط، وفي التطوير المحلي لا.
      secure: process.env.NODE_ENV === "production",
    });
    return res;
  } catch (e) {
    // إن كانت قاعدة البيانات فارغة أو الرابط غير صحيح نعرض سببًا واضحًا
    // بدل رسالة "تعذر تسجيل الدخول" العامة.
