    ...
    <div className="space-y-5">
        ...
        {PRESETS.map((p) => (
          <button
            ...
            className={cls(
              ...
              preset === p.key
                ? "border-[rgba(45,230,184,.55)] bg-[rgba(45,230,184,.12)] text-[var(--mint)]"
      ...
      {!data ? (
        ...
        <div className={cls("grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6", loading && "opacity-60 transition-opacity")}>
          <Stat icon={<Coins size={18} className="text-[var(--mint)]" />} tone="rgba(45,230,184,.12)" label="إجمالي المبيعات" value={fmtMoney(data.totals.total)} />
          <Stat icon={<TrendingUp size={18} className="text-[var(--amber)]" />} tone="rgba(255,194,75,.12)" label={`صافي الربح (هامش ${margin.toFixed(0)}%)`} value={fmtMoney(data.totals.profit)} />
          <Stat icon={<ReceiptText size={18} className="text-[var(--violet)]" />} tone="rgba(139,124,255,.12)" label="عدد الفواتير" value={fmtNum(data.totals.count)} />
          <Stat icon={<Scale size={18} className="text-[var(--sky)]" />} tone="rgba(86,182,247,.12)" label="متوسط الفاتورة" value={fmtMoney(data.totals.avg)} />
          <Stat icon={<Truck size={18} className="text-[var(--rose)]" />} tone="rgba(255,107,129,.12)" label="إجمالي الشحن" value={fmtMoney(data.totals.shipping)} />
          <Stat icon={<BarChart3 size={18} className="text-[var(--mint)]" />} tone="rgba(45,230,184,.12)" label="وحدات مباعة" value={fmtNum(data.totals.units)} />
        </div>
      )}
            ...
            <table className="tbl min-w-[430px]">
              ...
              <tbody>
                {data.topProducts.map((p, i) => (
                  <tr key={p.productId}>
                    <td className="w-8">
                      <span className={cls("inline-flex h-6 w-6 items-center justify-center rounded-lg text-[11px] font-black", i < 3 ? "bg-[rgba(45,230,184,.14)] text-[var(--mint)]" : "bg-white/5 text-[var(--faint)]")}>
                        {i + 1}
                      </span>
                    </td>
                    <td>
            ...
            <table className="tbl min-w-[430px]">
              ...
              <tbody>
                {data.topClients.map((c, i) => (
                  <tr key={c.clientId}>
                    <td className="w-8">
                      <span className={cls("inline-flex h-6 w-6 items-center justify-center rounded-lg text-[11px] font-black", i < 3 ? "bg-[rgba(139,124,255,.15)] text-[var(--violet)]" : "bg-white/5 text-[var(--faint)]")}>
                        {i + 1}
                      </span>
                    </td>
