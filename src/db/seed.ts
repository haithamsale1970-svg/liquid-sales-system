...
console.log(
  resolved
    ...
    : "→ لا يوجد متغيّر بيئة — سيتم استخدام قاعدة بيانات محلية على 127.0.0.1",
);

const db = drizzle(new Pool(buildPoolConfig(url)));
...
main().catch((e) => {
  console.error("فشلت التهيئة:", describeDbError(e));
  console.error(e);
  process.exit(1);
});
